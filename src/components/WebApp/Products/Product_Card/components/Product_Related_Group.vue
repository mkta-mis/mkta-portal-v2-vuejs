<template>
          ...
          <Products_ThumbnailVue :initClass='"col-12 col-s-12 col-md-3 col-lg-2"' :data="item" v-for="(item, index) in this.related" :key="index" />
</template>
<script>
...
export default {
          ...
          methods: {
                    get_Related_Data() {
                              this.$axios
                                        .post(this.$serverURL + "api/products/data/related/"+ this.code +"/group", {
                                        })
                                        .then((res) => {
